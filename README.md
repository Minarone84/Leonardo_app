...
AS-GUI-1 and AS-GUI-2 keep the GUI as an intent-collection and report-rendering layer. Backend services still own target generation, feature eligibility, leakage checks, dataframe reads, diagnostic composition, missingness/dtype checks, POI occurrence detection, family membership matching, source/condition validation, blockers, warnings, and errors. The GUI clears stale target/feature/diagnostic state when database, target, or feature selections change, and clears stale POI/family state when database, feature/diagnostic, POI definition, or family condition inputs change. It does not load `dataframe.csv` directly, parse `manifest.json` directly for readiness/feature/leakage/POI policy, compute POI occurrences or family memberships itself, persist target definitions, feature sets, diagnostic reports, POI definitions, or POI family definitions, expose AS9 genome path controls, expose category-builder UI, add stores, train models, generate signals, backtest, calculate artifacts, execute recipes, build/rebuild/materialize Analysis Databases, repair OHLCV, mutate Data Manager objects, edit manifests, or write dataframes. Initial AS-GUI-1 manual exploration found that workflow acceptable for continued work; layout and small usability polish remain future work.

Strict-ready still requires a readable manifest, materialized database, readable `dataframe.csv`, dataframe hash/metadata consistency when available, clean materialization source-OHLCV drift status, and complete minimum topology. Still out of scope: Analysis Projects/Runs/Reports, target/label/feature-set/diagnostic-report/POI/family/genome persistence, POI/family or genome stores, AS9 genome path GUI controls, GUI category-builder controls, road classification, outcome distribution measurement, full Dynamic Binner implementation or fitting, full Variation Analyzer implementation, white-box discovery, rule mining, backtesting, model training, signal generation, trading logic, artifact calculation, database build/rebuild/materialization, OHLCV repair, CoreBridge APIs, and manifest/dataframe/sidecar writes. Stale or incomplete datasets must be routed back to Data Manager or OHLCV Maintenance workflows instead of repaired inside Analysis Suite.
