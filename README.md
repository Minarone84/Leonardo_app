...
Analysis Suite is still not implemented as a GUI/product workflow. AS1 added the first read-only Analysis Suite backend boundary: `AnalysisSuiteDatasetReadinessService`, with JSON-safe `AnalysisSuiteDatasetReadinessReport` and `AnalysisSuiteDatasetCatalogReport` outputs. The service catalogs and evaluates Data Manager Analysis Databases before future Analysis Suite code loads `dataframe.csv`. It reports `ready`, `draft`, `missing_dataframe`, `stale_source`, `incomplete_topology`, `corrupt_manifest`, `corrupt_dataframe`, `blocked`, and `error` states. Strict-ready requires a readable manifest, materialized database, readable `dataframe.csv`, dataframe hash/metadata consistency when available, clean materialization source-OHLCV drift status, and complete minimum topology. AS1 scans manifest paths directly so corrupt manifests are reported, inspects dataframe row/column/timestamp metadata read-only, consumes `AnalysisDatasetGeographyPolicy` for topology, and consumes `AnalysisDatabaseStore.materialization_source_ohlcv_drift_report(...)` for source drift. It does not add an Analysis Suite window, Analysis Projects/Runs/Reports, model training, signal generation, artifact calculation, database build/rebuild/materialization, OHLCV repair, CoreBridge APIs, or any manifest/dataframe/sidecar writes. Stale or incomplete datasets must be routed back to Data Manager or OHLCV Maintenance workflows instead of repaired inside Analysis Suite.
