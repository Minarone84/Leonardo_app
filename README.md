...
- `DESIGN_analysis_suite.md` — Analysis Suite POI / event-family / road / genome / white-box rule architecture design.
...
AS6 rejects target output columns, label columns, `target_only`, `future_derived`, `feature_eligible = false`, the exact AS5 target output column, `ts_ms` as a normal feature, unknown metadata, and non-selectable or non-analysis-usable utility/internal columns. AS1 `can_preview` gates planning; non-strict but previewable datasets may still be planned with warnings/blockers preserved. Reports include candidates, selected/rejected features, group summaries, leakage summaries, blockers, warnings, and errors, and remain JSON-safe. AS6 remains non-persistent and does not create feature-set persistence, `FeatureSetStore`, Analysis Project/Run/Report stores, model training, signal generation, trading logic, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation/materialization.

AS7 added `AnalysisSuiteDiagnosticReportService`, a backend-only read-only diagnostic-report service with `AnalysisSuiteDiagnosticReport` and `AnalysisSuiteFeatureColumnDiagnostic` outputs. It composes the AS1 readiness report, AS5 target preview report, and AS6 feature-set preview report into a JSON-safe pre-analysis coherence report with final status values `ready`, `warning`, `blocked`, and `error`. The report summarizes dataset readiness, target coherence, feature-set validity, label availability, enough-row/label checks, regression stats or class distribution, selected-feature missingness and dtype diagnostics, leakage blockers, and blockers/warnings/errors.

AS7 is the checkpoint for whether a prepared Analysis Database plus target plus feature set is coherent enough for future analysis. It reads dataframe values only for AS6-accepted selected-feature consistency checks and does not treat raw CSV headers as feature truth. It remains non-persistent and does not create persisted diagnostic reports, Analysis Project/Run/Report stores, target/label/feature-set persistence, model training, signal generation, trading logic, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation/materialization. It is the final pre-analysis diagnostic checkpoint before POI / event-family / road / genome architecture, not POI analysis or white-box discovery.

AS8-AUDIT documents POI / event-family / road / genome architecture in `docs/DESIGN_analysis_suite.md`. It defines the World Line, POIs, event families/subfamilies, roads, outcomes, false-signal classes, genomes, genome paths, event windows, AS1-AS7 relationships, and the AS8 backend MVP boundary.

AS8 added `AnalysisSuitePoiFamilyPlanner`, a backend-only read-only POI/family planner with `AnalysisSuitePoiDefinition`, `AnalysisSuitePoiCondition`, `AnalysisSuitePoiFamilyDefinition`, `AnalysisSuitePoiOccurrence`, `AnalysisSuitePoiFamilyMembership`, `AnalysisSuitePoiOccurrencePreviewReport`, and `AnalysisSuitePoiFamilyPreviewReport`. It previews POI occurrences from prepared Analysis Database columns and POI family membership through simple AND-style same-row or fixed-lookback conditions. Supported POI event kinds are `sparse_event`, `boolean_true`, `value_equals`, and `transition`; supported condition operators are `equals`, `not_equals`, `gt`, `gte`, `lt`, `lte`, `in`, `not_in`, `is_null`, and `not_null`. AS8 uses AS1 readiness or optional AS7 diagnostic gating, blocks `can_preview == false` and blocked/error AS7 diagnostics, allows non-strict but previewable datasets with diagnostics preserved, validates source/condition columns through manifest metadata before dataframe reads, rejects target-only/future-derived/`feature_eligible = false` inputs when metadata is available, returns JSON-safe bounded samples with default `100` and max `500`, and does not treat raw CSV headers as event truth.

AS9 added `AnalysisSuiteGenomePathBuilder`, a backend-only read-only genome/path planner with `AnalysisSuiteStaticBinRule`, `AnalysisSuiteGenomeComponentDefinition`, `AnalysisSuiteGenomeEncodingDefinition`, `AnalysisSuiteGenomeSnapshot`, `AnalysisSuiteGenomePath`, and `AnalysisSuiteGenomePathPreviewReport`. It supports `validate_encoding_definition(...)`, `preview_paths(...)`, and `preview_paths_for_poi_family(...)`; conservative encodings `identity_numeric`, `categorical`, `boolean_symbolic`, `static_bin`, and `variation_direction`; row-anchored paths `G(t-k) ... G(t)`; and optional AS8 POI/family anchoring through matched family membership samples. AS9 uses AS1 readiness or AS7 diagnostic gating, blocks `can_preview == false` and blocked/error AS7 diagnostics, allows non-strict but previewable datasets with warnings preserved, uses manifest metadata or AS6-style feature metadata as semantic column truth where available, blocks `ts_ms`, target-only, future-derived, and `feature_eligible = false` inputs when metadata is available, reads dataframe values only for read-only genome/path preview, returns JSON-safe bounded samples with default `100` and max `500`, and does not treat raw CSV headers as semantic genome truth.

AS-GUI-1 added the first interactive Analysis Suite setup workflow in `AnalysisSuiteWindow`. The window contains a tabbed read-only surface with `Data Preview`, `Target Preview`, `Feature Set`, and `Diagnostic Report`. `Data Preview` preserves the existing AS3 bounded Head/Tail dataframe preview. `Target Preview` collects target family, `horizon_bars`, and direction thresholds, calls AS5 `AnalysisSuiteTargetPlanner`, and renders label availability, target stats or class distribution, leakage metadata, blockers, warnings, and errors. `Feature Set` lists AS6 candidates, shows status/group/reason metadata, supports selected-feature validation with `Select All Eligible`, calls `AnalysisSuiteFeatureSetPlanner`, and renders selected/rejected features, group summaries, leakage summaries, blockers, warnings, and errors. `Diagnostic Report` requires a selected database plus current target and feature-set previews, calls AS7 `AnalysisSuiteDiagnosticReportService`, and renders ready/warning/blocked/error status plus dataset, target, feature, leakage, label, missingness, dtype, blocker, warning, and error diagnostics.

AS-GUI-2 added the read-only `POI / Family Preview` tab to `AnalysisSuiteWindow`. The POI section collects POI key, display name, POI type, source column, event kind, event value, and previous value; exposes the AS8 event kinds `sparse_event`, `boolean_true`, `value_equals`, and `transition`; calls `AnalysisSuitePoiFamilyPlanner.preview_poi_occurrences(...)`; and renders status, row count, occurrence count, first/last occurrence timestamps, sample occurrences, blockers, warnings, and errors. The Family Conditions section collects family key/name and simple AND-style condition rows with column, operator, value/values, `lookback_bars`, required flag, and label; accepts AS8 condition operators `equals`, `not_equals`, `gt`, `gte`, `lt`, `lte`, `in`, `not_in`, `is_null`, and `not_null`; calls `AnalysisSuitePoiFamilyPlanner.preview_family(...)`; and renders occurrence count, matched/unmatched counts, sample memberships, condition result summaries, blockers, warnings, and errors.

AS-GUI-3 added the read-only `Genome Path Preview` tab to `AnalysisSuiteWindow`. The Encoding Definition section collects encoding key/name, `path_length_bars`, anchor mode, and genome component rows with enabled state, component key, source column, encoding type, static bin config, `lookback_bars`, missing token, and display name. It exposes AS9 MVP encodings `identity_numeric`, `categorical`, `boolean_symbolic`, `static_bin`, and `variation_direction`; calls `AnalysisSuiteGenomePathBuilder.validate_encoding_definition(...)`; and renders validation blockers, warnings, and errors. Row-anchored preview calls `AnalysisSuiteGenomePathBuilder.preview_paths(...)`; POI-family-anchored preview requires current AS8 family preview context and calls `AnalysisSuiteGenomePathBuilder.preview_paths_for_poi_family(...)`. The tab renders status, row/path counts, sample paths, sample snapshots/components, blockers, warnings, and errors.

AS10-AUDIT documents the architecture for POI family comparison and white-box rule diagnostics. It defines white-box rules as explicit human-readable predicates over AS9 genome components and AS8 POI/family context, rule candidates as tested condition sets with support/precision/recall/lift/stability diagnostics, and comparison sets as positive and negative/background cohorts with inclusion, exclusion, anchor, sample-count, time-range, and leakage constraints. It also defines road, outcome, and false-signal analysis concepts, rule-testing versus rule-discovery boundaries, and the backend-only AS10 rule-testing MVP boundary.

AS10 added `AnalysisSuiteWhiteBoxRuleTester`, a backend-only read-only rule-testing service with `AnalysisSuiteRulePredicate`, `AnalysisSuiteWhiteBoxRuleDefinition`, `AnalysisSuiteComparisonCohort`, `AnalysisSuiteComparisonSetDefinition`, `AnalysisSuitePredicateResult`, `AnalysisSuiteRuleMatch`, `AnalysisSuiteRuleMetricSummary`, `AnalysisSuiteRuleValidationReport`, and `AnalysisSuiteRuleTestReport`. Public methods are `validate_rule_definition(...)`, `test_rule(...)`, and `build_comparison_set_from_path_reports(...)`. AS10 evaluates explicit AND-style predicates over supplied AS9 genome paths and explicit positive/negative comparison cohorts. Supported predicate operators are `equals`, `not_equals`, `gt`, `gte`, `lt`, `lte`, `in`, `not_in`, `is_null`, and `not_null`; `path_offset = 0` means the anchor snapshot, negative offsets address previous snapshots, and positive offsets are blocked to prevent future-snapshot leakage. Reports include predicate results, rule matches, true/false positive and negative counts, support, coverage, precision, recall, specificity, false-positive/false-negative rates, baseline positive rate, matched positive rate, lift, small-sample warnings, AS7 diagnostic gating, and bounded-preview sample-limit metadata. AS10 metrics are evidence-quality diagnostics, not trading performance. AS10 does not discover broad rule candidates, perform candidate mining, expose GUI controls, persist rules/comparison sets/reports, read `dataframe.csv` directly, parse `manifest.json` for policy, train models, generate signals, backtest, validate PnL/profit, trade, calculate artifacts, execute recipes, mutate Analysis Databases or Data Manager objects, or repair OHLCV.

AS11-AUDIT documents bounded white-box rule candidate discovery architecture. It defines candidate discovery as diagnostic proposal generation from a finite AS9-derived predicate vocabulary, with single-predicate scans as the recommended first backend MVP, cautious future conjunction expansion, support-first ranking, pruning rules, multiple-testing and false-discovery warnings, temporal stability concepts, family separation diagnostics, explainability requirements, and AS8/AS9/AS10 ownership boundaries.

AS11 added `AnalysisSuiteRuleCandidateScanner`, a backend-only read-only bounded candidate scan service with `AnalysisSuitePredicateVocabularyItem`, `AnalysisSuitePredicateVocabularyReport`, `AnalysisSuiteRuleCandidate`, `AnalysisSuiteRuleCandidateScanConfig`, and `AnalysisSuiteRuleCandidateScanReport`. AS11 builds a finite single-predicate vocabulary from observed AS9 genome path components inside AS10 comparison cohorts. Symbolic, boolean, and bin-like observed values become `equals` predicates by default; configured scans may include `not_equals`, `is_null`, and `not_null`. Numeric threshold mining is skipped by default when `numeric_threshold_policy = "none"`. Positive path offsets are blocked, negative offsets are allowed only when explicitly configured, and components marked leaky or future-derived by metadata are skipped. Each vocabulary item becomes one AS10 `AnalysisSuiteWhiteBoxRuleDefinition`, and AS11 calls `AnalysisSuiteWhiteBoxRuleTester.test_rule(...)` for matching and metrics instead of duplicating AS10 formulas. Reports rank bounded candidates with a diagnostic lift/precision/support-weight score, filter by blockers, `min_total_support`, `min_positive_matches`, optional `min_precision`, and optional `min_lift`, and include `candidate_count_scanned`, `candidate_count_returned`, `candidate_count_filtered`, sample-limited warnings, a no-multiple-testing-correction warning, and `stability_status = "unknown"` because temporal stability validation is not performed in AS11 MVP. AS11 does not perform conjunction expansion, broad mining, itemset mining, recursive search, genetic programming, GUI wiring, persistence, stores, model training, signals, backtesting, PnL/profit validation, trading execution, artifact calculation, recipe execution, direct dataframe or manifest policy reads, database mutation, Data Manager mutation, or OHLCV repair.

AS12 added `AnalysisSuiteCandidateTemporalValidator`, a backend-only read-only candidate temporal validation service with `AnalysisSuiteTemporalValidationSegment`, `AnalysisSuiteTemporalValidationConfig`, `AnalysisSuiteCandidateSegmentValidation`, `AnalysisSuiteCandidateMetricDegradation`, `AnalysisSuiteCandidateTemporalValidationResult`, and `AnalysisSuiteCandidateTemporalValidationReport`. AS12 validates already-discovered AS11 candidates across chronological discovery/validation segments, builds `chronological_holdout` splits from `anchor_ts_ms`, preserves positive/negative cohort roles, and retests candidate rules through AS10 `AnalysisSuiteWhiteBoxRuleTester.test_rule(...)` for segment-level metrics instead of duplicating AS10 metric formulas. Reports include metric degradation diagnostics, a diagnostic stability score, survival statuses (`survived`, `degraded`, `failed`, `insufficient_data`, `unknown`), AS11 multiple-testing warning propagation, sample-limited warning propagation, and bounded candidate counts (`candidate_count_input`, `candidate_count_validated`, `candidate_count_survived`, `candidate_count_degraded`, `candidate_count_failed`, `candidate_count_insufficient_data`, `candidate_count_unknown`) with default `100` / max `500` validated candidates. AS12 validates candidate robustness only; it is not profitability validation, backtesting, PnL validation, signal generation, trading execution, candidate rescanning, new candidate mining, or conjunction expansion.

AS-GUI-1, AS-GUI-2, and AS-GUI-3 keep the GUI as an intent-collection and report-rendering layer. Backend services still own target generation, feature eligibility, leakage checks, dataframe reads, diagnostic composition, missingness/dtype checks, POI occurrence detection, family membership matching, source/condition validation, genome component validation, genome encoding, path construction, white-box rule testing, candidate vocabulary generation, candidate scanning, candidate temporal validation, blockers, warnings, and errors. The GUI clears stale target/feature/diagnostic state when database, target, or feature selections change; clears stale POI/family state when database, feature/diagnostic, POI definition, or family condition inputs change; and clears stale genome reports when database, feature, diagnostic, POI, family, or encoding inputs change. It does not load `dataframe.csv` directly, parse `manifest.json` directly for readiness/feature/leakage/POI/genome/rule policy, compute POI occurrences, family memberships, genome snapshots, genome paths, variation descriptors, white-box rule metrics, candidate scans, candidate temporal validation, validation cohort splits, metric degradation, or stability scores itself, fit bins, persist target definitions, feature sets, diagnostic reports, POI definitions, POI family definitions, genome encoding definitions, genome paths, rule definitions, comparison sets, rule reports, scan configs, candidate reports, or temporal validation reports, expose AS10/AS11/AS12 white-box or validation controls, expose category-builder UI, add stores, train models, generate signals, backtest, calculate artifacts, execute recipes, build/rebuild/materialize Analysis Databases, repair OHLCV, mutate Data Manager objects, edit manifests, or write dataframes. Initial AS-GUI-1 manual exploration found that workflow acceptable for continued work; layout and small usability polish remain future work.

Strict-ready still requires a readable manifest, materialized database, readable `dataframe.csv`, dataframe hash/metadata consistency when available, clean materialization source-OHLCV drift status, and complete minimum topology. Still out of scope: Analysis Projects/Runs/Reports, target/label/feature-set/diagnostic-report/POI/family/genome/rule/candidate/temporal-validation persistence, POI/family/genome/rule/candidate/validation stores, AS10/AS11/AS12 GUI controls, GUI category-builder controls, implemented road classification, outcome distribution measurement, Dynamic Binner fitting, full Dynamic Binner implementation, full Variation Analyzer implementation, broad white-box discovery, conjunction expansion, broad candidate mining, itemset mining, recursive search, genetic programming, rule mining, temporal validation beyond the AS12 chronological-holdout MVP, backtesting, PnL/profit validation, model training, signal generation, trading logic, artifact calculation, database build/rebuild/materialization, OHLCV repair, CoreBridge APIs, and manifest/dataframe/sidecar writes. Stale or incomplete datasets must be routed back to Data Manager or OHLCV Maintenance workflows instead of repaired inside Analysis Suite.
