...
Data Manager Construct Batch Builder uses the same financial-tool contract and persistence boundaries for construct recipe expansion. Generic batch supports unary `derivative`, `angle`, `percent_span_angle`, and `angle_momentum`, plus binary `delta` reported as `delta = minuend - subtrahend`. The planner builds expected recipe payload/output previews through existing specs and naming helpers, checks saved-artifact source eligibility through `selectable` / `analysis_usable` metadata, and requires timestamp-safe alignment/common ranges rather than equal row count. `braids`, `braid_instability`, `trap_area`, and `dynamic_binning` are not generic batch constructs; topology-template and grouped-analysis workflows remain separate future designs. Preview Plan writes nothing. Save Recipes persists selected planned recipes through `ArtifactRecipeStore`; Save as Collection persists/reuses recipes before saving an ordered collection through `ArtifactRecipeCollectionStore`; Calculate Artifacts persists/reuses recipes first and executes them sequentially through `ArtifactRecipeExecutor` / `ArtifactCalculationService`. The GUI displays reports and refreshes lists, but it does not calculate directly, write CSV/sidecars directly, or modify Analysis Databases.
