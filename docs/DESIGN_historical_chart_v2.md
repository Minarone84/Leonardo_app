...
This metadata is semantic context for saved research workflows. Chart-applied study controls are reserved for chart-local actions such as style, edit, and remove. Study Environment save/update is the boundary where per-study environment metadata is selected. The dialog applies `important`, `dataset_role`, and `description` to cloned serialized study payloads before store-owned persistence. It does not mutate the live chart registry because serialized save payloads do not carry durable chart-session instance identifiers. Metadata is preserved by study serialization/deserialization, Study Environments, Workspace Snapshots, and computation edit/reapply. It must not affect computation, resident projection, rendering, style, runtime render keys, artifact identity, recipe identity, or dataset geography truth. `dataset_role` is a hint for review/reporting, not proof of tool identity.
