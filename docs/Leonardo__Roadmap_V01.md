...
- AS-GUI-AUDIT documented the Analysis Suite GUI exposure boundary. It confirms `AnalysisSuiteWindow` currently exposes only the AS1 readiness catalog, AS3 bounded dataframe preview, Data Manager routing, refresh, and close actions. It recommends AS-GUI-1 as the next implementation patch: Target / Feature / Diagnostic Preview UI exposing AS5, AS6, and AS7 together as a read-only in-memory setup workflow. AS8 POI/family controls, AS9 genome/path controls, persistence, stores, model logic, signals, backtesting, white-box discovery, artifact calculation, recipe execution, OHLCV repair, and Analysis Database mutation remain out of scope.
...
- Treat Analysis Suite AS1-AS9 as the accepted read-only foundation for readiness, bounded dataframe preview, target/label preview, feature-set leakage prevention, diagnostic setup coherence, backend POI/family occurrence preview, and backend genome/path preview.
- Treat AS8-AUDIT and AS9-AUDIT as the accepted design boundary for broader POI / event-family / road / genome architecture.
- Treat AS-GUI-AUDIT as the accepted GUI exposure boundary for Analysis Suite. Next Analysis Suite step: AS-GUI-1 - Target / Feature / Diagnostic Preview UI, exposing AS5 target preview, AS6 feature-set preview, and AS7 diagnostic report preview together as a read-only non-persistent setup workflow. AS-GUI-2 should later expose AS8 POI/family previews, AS-GUI-3 should later expose AS9 genome path previews, and AS10-AUDIT for POI Family Comparison and White-Box Rule Discovery should follow after the GUI can inspect current backend setup objects.
...
Change log
------------------------------------------------------------
v0.43: Analysis Suite GUI audit sync. Documents AS-GUI-AUDIT as the accepted GUI exposure boundary, confirms current `AnalysisSuiteWindow` remains a read-only AS1 catalog plus AS3 bounded preview surface, recommends AS-GUI-1 as Target / Feature / Diagnostic Preview UI for AS5/AS6/AS7, stages AS-GUI-2 for AS8 POI/family previews and AS-GUI-3 for AS9 genome path previews, and preserves no-GUI-implementation/no-persistence/no-stores/no-model/no-signal/no-database-mutation boundaries.
