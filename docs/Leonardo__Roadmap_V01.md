...
- Treat AS8-AUDIT and AS9-AUDIT as the accepted design boundary for broader POI / event-family / road / genome architecture.
- Treat AS-GUI-AUDIT as the accepted GUI exposure boundary, AS-GUI-1 as the accepted target/feature/diagnostic setup GUI, AS-GUI-2 as the accepted POI/family preview GUI, and AS-GUI-3 as the accepted genome path preview GUI. Next Analysis Suite step after AS-GUI-3D: AS10-AUDIT for POI Family Comparison and White-Box Rule Discovery. GUI polish may be handled separately if manual exploration finds usability issues.
