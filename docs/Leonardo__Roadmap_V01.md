...
- AS8-AUDIT added the design-only `docs/DESIGN_analysis_suite.md` architecture for POIs, event families/subfamilies, roads, outcomes, false-signal classes, genomes, genome paths, event windows, and the future backend-only POI Definition and Family Planner MVP. It does not implement backend services, GUI controls, persistence, POI discovery, white-box discovery, backtesting, model logic, signals, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.
...
- Treat Analysis Suite AS1-AS7 as the accepted read-only foundation for readiness, bounded dataframe preview, target/label preview, feature-set leakage prevention, and diagnostic setup coherence.
- Treat AS8-AUDIT as the accepted design boundary for POI / event-family / road / genome architecture.
- Next Analysis Suite implementation step: AS8 POI Definition and Family Planner Backend. White-box rule discovery, Research Suite validation integration, neural refinement, and Decisor logic remain future work.
