...
- Treat Analysis Suite AS1-AS8 as the accepted read-only foundation for readiness, bounded dataframe preview, target/label preview, feature-set leakage prevention, diagnostic setup coherence, and backend POI/family occurrence preview.
- Treat AS8-AUDIT and AS9-AUDIT as the accepted design boundary for broader POI / event-family / road / genome architecture.
- Next Analysis Suite implementation step: AS9 Genome Path Builder Backend. It should remain backend-only, read-only, non-persistent, GUI-free, consume AS6/AS7/AS8 report context, build bounded JSON-safe genome snapshot/path previews, and avoid future leakage. AS10-AUDIT for POI Family Comparison and White-Box Rule Discovery should follow after AS9 behavior is accepted. AS-GUI-AUDIT remains future work for deciding how backend-only target, feature-set, diagnostic, POI/family, and genome-path planners should be exposed later.
