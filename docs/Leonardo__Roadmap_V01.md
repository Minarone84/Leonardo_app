...
- AS-GUI-2 added POI / Family Preview UI to `AnalysisSuiteWindow` as a read-only in-memory AS8 workflow. The window now also has a `POI / Family Preview` tab. The POI section collects POI key, display name, POI type, source column, event kind, event value, and previous value; exposes AS8 event kinds `sparse_event`, `boolean_true`, `value_equals`, and `transition`; calls `AnalysisSuitePoiFamilyPlanner.preview_poi_occurrences(...)`; and renders occurrence counts, sample occurrences, blockers, warnings, and errors. The Family Conditions section collects family key/name and simple AND-style condition rows with column, operator, value/values, `lookback_bars`, required flag, and label; accepts AS8 condition operators; calls `AnalysisSuitePoiFamilyPlanner.preview_family(...)`; and renders matched/unmatched counts, sample memberships, condition summaries, blockers, warnings, and errors. The GUI clears stale POI/family state when database, feature/diagnostic, POI definition, or family condition inputs change. It does not compute POI occurrences or family memberships itself, read `dataframe.csv` directly, parse `manifest.json` directly for policy, persist POI/family definitions, expose AS9 genome path controls, expose category-builder UI, create stores, train models, generate signals, backtest, calculate artifacts, execute recipes, repair OHLCV, or mutate Analysis Databases.
...
- Treat Analysis Suite AS1-AS9 plus AS-GUI-1 and AS-GUI-2 as the accepted read-only foundation for readiness, bounded dataframe preview, target/label preview, feature-set leakage prevention, diagnostic setup coherence, GUI target/feature/diagnostic setup inspection, backend and GUI POI/family occurrence preview, and backend genome/path preview.
- Treat AS8-AUDIT and AS9-AUDIT as the accepted design boundary for broader POI / event-family / road / genome architecture.
- Treat AS-GUI-AUDIT as the accepted GUI exposure boundary, AS-GUI-1 as the accepted target/feature/diagnostic setup GUI, and AS-GUI-2 as the accepted POI/family preview GUI. Next Analysis Suite step after AS-GUI-2D: either AS-GUI-3 for AS9 genome path previews or AS10-AUDIT for POI Family Comparison and White-Box Rule Discovery. AS-GUI-3 is not implemented yet.
