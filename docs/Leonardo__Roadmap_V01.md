...
- AS5 added the backend-only read-only `AnalysisSuiteTargetPlanner` with `AnalysisSuiteTargetDefinition` and `AnalysisSuiteTargetPreviewReport`. It previews future return regression and future direction classification labels in memory, uses positive `horizon_bars` alignment back to timestamp `t`, marks the last `N` rows unavailable, gates through AS1 `can_preview`, and reports leakage metadata marking label outputs as `target_only`, `future_derived`, and not feature-eligible. It does not add target/label persistence, projects/runs/reports, model logic, signals, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.
- AS6 added the backend-only read-only `AnalysisSuiteFeatureSetPlanner` with `AnalysisSuiteFeatureCandidate`, `AnalysisSuiteFeatureSetDefinition`, and `AnalysisSuiteFeatureSetPreviewReport`. It lists manifest-derived feature candidates, validates selected feature columns while preserving order, gates through AS1 `can_preview`, rejects AS5 leakage metadata and exact target output columns, reserves `ts_ms` as an alignment key, blocks unknown metadata in MVP, keeps current-row `close[t]` eligible for future-return targets when otherwise valid, and returns JSON-safe group/leakage diagnostics. It does not add feature-set persistence, `FeatureSetStore`, projects/runs/reports, model logic, signals, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.
- AS7 added the backend-only read-only `AnalysisSuiteDiagnosticReportService` with `AnalysisSuiteDiagnosticReport` and `AnalysisSuiteFeatureColumnDiagnostic`. It composes AS1 readiness, AS5 target preview, and AS6 feature-set preview reports into JSON-safe diagnostic reports with `ready`, `warning`, `blocked`, and `error` statuses. It reports dataset readiness, target coherence, feature-set validity, label availability, enough-row/label checks, target stats or class distribution, selected-feature missingness/dtype summaries, leakage blockers, and combined blockers/warnings/errors. It reads dataframe values only for AS6-accepted selected-feature consistency checks, does not treat raw CSV headers as feature truth, and remains a pre-analysis checkpoint before POI / event-family / road / genome architecture. It does not add persisted diagnostic reports, projects/runs/reports, model logic, signals, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.
- AS8-AUDIT added the design-only `docs/DESIGN_analysis_suite.md` architecture for POIs, event families/subfamilies, roads, outcomes, false-signal classes, genomes, genome paths, event windows, and the backend-only POI Definition and Family Planner boundary.
- AS8 added the backend-only read-only `AnalysisSuitePoiFamilyPlanner` with `AnalysisSuitePoiDefinition`, `AnalysisSuitePoiCondition`, `AnalysisSuitePoiFamilyDefinition`, `AnalysisSuitePoiOccurrence`, `AnalysisSuitePoiFamilyMembership`, `AnalysisSuitePoiOccurrencePreviewReport`, and `AnalysisSuitePoiFamilyPreviewReport`. It supports `preview_poi_occurrences(...)`, `preview_family(...)`, and `validate_family_definition(...)`; event kinds `sparse_event`, `boolean_true`, `value_equals`, and `transition`; and condition operators `equals`, `not_equals`, `gt`, `gte`, `lt`, `lte`, `in`, `not_in`, `is_null`, and `not_null`. It gates through AS1 readiness or optional AS7 diagnostics, blocks non-previewable or blocked/error diagnostic contexts, allows non-strict previewable datasets with warnings/blockers preserved, validates source/condition columns through manifest metadata before dataframe reads, rejects target-only/future-derived/not-feature-eligible inputs when metadata exists, and returns bounded JSON-safe POI occurrence and family membership previews with default `100` / max `500` samples. It does not add GUI wiring, persistence, POI stores, projects/runs/reports, road classification, genome paths, Dynamic Binner or Variation Analyzer implementation, white-box discovery, backtesting, model logic, signals, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.
- AS9-AUDIT expanded `docs/DESIGN_analysis_suite.md` with Genome Encoding / Variation Analyzer / Dynamic Binner integration architecture. It defines genome components, encoding policies, genome snapshots, genome paths, Variation Analyzer and Dynamic Binner roles, leakage and knowability rules, AS5/AS6/AS7/AS8 relationships, and the recommended backend-only AS9 Genome Path Builder boundary. It does not implement backend services, GUI controls, persistence, stores, white-box rule discovery, backtesting, model logic, signals, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.
- AS9 added the backend-only read-only `AnalysisSuiteGenomePathBuilder` with `AnalysisSuiteStaticBinRule`, `AnalysisSuiteGenomeComponentDefinition`, `AnalysisSuiteGenomeEncodingDefinition`, `AnalysisSuiteGenomeSnapshot`, `AnalysisSuiteGenomePath`, and `AnalysisSuiteGenomePathPreviewReport`. It supports `validate_encoding_definition(...)`, `preview_paths(...)`, and `preview_paths_for_poi_family(...)`; encodings `identity_numeric`, `categorical`, `boolean_symbolic`, `static_bin`, and `variation_direction`; row-anchored paths `G(t-k) ... G(t)`; and optional AS8 POI/family anchoring through matched family membership samples. It gates through AS1 readiness or optional AS7 diagnostics, blocks non-previewable or blocked/error diagnostic contexts, allows non-strict previewable datasets with warnings preserved, validates component source columns through AS6 feature-set reports or manifest metadata when available, blocks `ts_ms`, target-only, future-derived, and not-feature-eligible inputs when metadata exists, reads dataframe values only for read-only genome/path preview, and returns bounded JSON-safe samples with default `100` / max `500`. It does not add GUI wiring, persistence, genome stores, projects/runs/reports, Dynamic Binner fitting, full Dynamic Binner, full Variation Analyzer, white-box discovery, rule mining, backtesting, model logic, signals, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.
- AS-GUI-AUDIT documented the Analysis Suite GUI exposure boundary. It confirmed the pre-AS-GUI-1 `AnalysisSuiteWindow` baseline as the AS1 readiness catalog, AS3 bounded dataframe preview, Data Manager routing, refresh, and close actions, and recommended AS-GUI-1 as the first implementation patch.
- AS-GUI-1 added Target / Feature / Diagnostic Preview UI to `AnalysisSuiteWindow` as a read-only in-memory setup workflow. The window now has `Data Preview`, `Target Preview`, `Feature Set`, and `Diagnostic Report` tabs. Target Preview calls AS5 `AnalysisSuiteTargetPlanner`; Feature Set calls AS6 `AnalysisSuiteFeatureSetPlanner`; Diagnostic Report calls AS7 `AnalysisSuiteDiagnosticReportService`. The GUI collects intent, renders backend reports, clears stale target/feature/diagnostic state when database/target/feature selections change, and keeps backend services as the owners of dataframe reads, target generation, feature eligibility, leakage policy, diagnostics, blockers, warnings, and errors. It does not read `dataframe.csv` directly, parse `manifest.json` directly for policy, persist target definitions, persist feature sets, persist diagnostic reports, expose AS8 POI/family controls, expose AS9 genome path controls, create stores, train models, generate signals, backtest, calculate artifacts, execute recipes, repair OHLCV, or mutate Analysis Databases. Initial manual GUI exploration found the workflow acceptable for continued work; layout and small polish remain future work.
- AS-GUI-2 added POI / Family Preview UI to `AnalysisSuiteWindow` as a read-only in-memory AS8 workflow. The window now also has a `POI / Family Preview` tab. The POI section collects POI key, display name, POI type, source column, event kind, event value, and previous value; exposes AS8 event kinds `sparse_event`, `boolean_true`, `value_equals`, and `transition`; calls `AnalysisSuitePoiFamilyPlanner.preview_poi_occurrences(...)`; and renders occurrence counts, sample occurrences, blockers, warnings, and errors. The Family Conditions section collects family key/name and simple AND-style condition rows with column, operator, value/values, `lookback_bars`, required flag, and label; accepts AS8 condition operators; calls `AnalysisSuitePoiFamilyPlanner.preview_family(...)`; and renders matched/unmatched counts, sample memberships, condition summaries, blockers, warnings, and errors. The GUI clears stale POI/family state when database, feature/diagnostic, POI definition, or family condition inputs change. It does not compute POI occurrences or family memberships itself, read `dataframe.csv` directly, parse `manifest.json` directly for policy, persist POI/family definitions, expose category-builder UI, create stores, train models, generate signals, backtest, calculate artifacts, execute recipes, repair OHLCV, or mutate Analysis Databases.
- AS-GUI-3 added Genome Path Preview UI to `AnalysisSuiteWindow` as a read-only in-memory AS9 workflow. The window now also has a `Genome Path Preview` tab. The Encoding Definition section collects encoding key/name, `path_length_bars`, anchor mode, and genome component rows with enabled state, component key, source column, encoding type, static bin config, `lookback_bars`, missing token, and display name; exposes AS9 encodings `identity_numeric`, `categorical`, `boolean_symbolic`, `static_bin`, and `variation_direction`; calls `AnalysisSuiteGenomePathBuilder.validate_encoding_definition(...)`; and renders validation blockers, warnings, and errors. Row-anchored preview calls `AnalysisSuiteGenomePathBuilder.preview_paths(...)`; POI-family-anchored preview requires a current AS8 family preview report and calls `AnalysisSuiteGenomePathBuilder.preview_paths_for_poi_family(...)`. The GUI renders status, row/path counts, sample paths, snapshots/components, blockers, warnings, and errors; clears stale genome state when database, feature, diagnostic, POI, family, or encoding inputs change; and does not compute genome snapshots, compute genome paths, compute variation descriptors, fit bins, persist genome definitions/paths, expose AS10 controls, expose white-box/rule discovery, train models, generate signals, backtest, calculate artifacts, execute recipes, repair OHLCV, or mutate Analysis Databases.
- AS10-AUDIT documents POI Family Comparison and White-Box Rule Discovery architecture. It defines white-box rules as explicit human-readable predicates over AS9 genome components and AS8 POI/family context, rule candidates as tested condition sets with support/precision/recall/lift/stability diagnostics, comparison sets as positive and negative/background cohorts, road/outcome/false-signal analysis concepts, diagnostic metrics, rule-testing versus rule-discovery boundaries, and the recommended backend-only AS10 rule-testing MVP. It does not implement backend services, GUI controls, persistence, stores, rule mining, backtesting, PnL/profit validation, model training, signals, trading execution, artifact calculation, recipe execution, Analysis Database mutation, or OHLCV repair.
- AS10 added the backend-only read-only `AnalysisSuiteWhiteBoxRuleTester` with `AnalysisSuiteRulePredicate`, `AnalysisSuiteWhiteBoxRuleDefinition`, `AnalysisSuiteComparisonCohort`, `AnalysisSuiteComparisonSetDefinition`, `AnalysisSuitePredicateResult`, `AnalysisSuiteRuleMatch`, `AnalysisSuiteRuleMetricSummary`, `AnalysisSuiteRuleValidationReport`, and `AnalysisSuiteRuleTestReport`. It supports `validate_rule_definition(...)`, `test_rule(...)`, and `build_comparison_set_from_path_reports(...)`; tests explicit AND-style predicates over AS9 genome paths and explicit positive/negative cohorts; supports operators `equals`, `not_equals`, `gt`, `gte`, `lt`, `lte`, `in`, `not_in`, `is_null`, and `not_null`; blocks positive path offsets; preserves AS7 diagnostic gating; reports support, coverage, precision, recall, specificity, false-positive/false-negative rates, baseline/matched positive rates, lift, and bounded-preview sample-limit metadata; and does not add GUI controls, persistence, stores, broad rule discovery, candidate mining, model logic, signals, backtesting, PnL/profit validation, trading execution, artifact calculation, recipe execution, direct dataframe/manifest policy reads, Analysis Database mutation, or OHLCV repair.
- AS11-AUDIT documents White-Box Rule Candidate Discovery / Bounded Mining architecture. It defines bounded candidate discovery as diagnostic proposal generation from finite AS9-derived predicate vocabulary, with single-predicate scans as the recommended first backend MVP, cautious future conjunction expansion, support-first ranking, pruning rules, multiple-testing and false-discovery warnings, temporal split and stability concepts, family separation diagnostics, explainability requirements, and AS8/AS9/AS10 ownership boundaries. It does not implement backend services, GUI controls, persistence, stores, candidate scanning, broad conjunction mining, backtesting, PnL/profit validation, model training, signals, trading execution, artifact calculation, recipe execution, Analysis Database mutation, OHLCV repair, or Data Manager mutation.
- AS11 added the backend-only read-only `AnalysisSuiteRuleCandidateScanner` with `AnalysisSuitePredicateVocabularyItem`, `AnalysisSuitePredicateVocabularyReport`, `AnalysisSuiteRuleCandidate`, `AnalysisSuiteRuleCandidateScanConfig`, and `AnalysisSuiteRuleCandidateScanReport`. It builds bounded single-predicate vocabularies from AS9 genome path components inside AS10 comparison cohorts, defaults symbolic/bool/bin-like observed values to `equals` predicates, optionally supports `not_equals`, `is_null`, and `not_null`, skips numeric threshold mining by default when `numeric_threshold_policy = "none"`, blocks positive path offsets, supports negative offsets only when explicitly enabled, skips leaky/future-derived metadata components, converts each vocabulary item into a one-predicate AS10 rule, reuses `AnalysisSuiteWhiteBoxRuleTester.test_rule(...)` for matching and metrics, filters by blockers/support/positive matches/optional precision/optional lift, ranks bounded candidates with a diagnostic lift/precision/support-weight score, reports scanned/returned/filtered counts, sample-limited warnings, no multiple-testing correction, and `stability_status = "unknown"`, and does not add GUI controls, persistence, stores, conjunction expansion, broad mining, recursive search, itemset mining, genetic programming, model logic, signals, backtesting, PnL/profit validation, trading execution, artifact calculation, recipe execution, direct dataframe/manifest policy reads, Analysis Database mutation, OHLCV repair, or Data Manager mutation.
- AS12-AUDIT documents Candidate Validation / Temporal Stability architecture. It defines diagnostic retesting of AS11 candidates across chronological segments, `chronological_holdout` as the recommended first validation split, validation split types, rule survival statuses, metric degradation, stability scoring, segment-level reports, AS10/AS11 ownership relationships, sample-limited validation warnings, multiple-testing risk preservation, and candidate promotion/demotion statuses. It does not implement backend services, GUI controls, persistence, stores, saved validated rules, model training, signals, backtesting, PnL/profit validation, trading execution, artifact calculation, recipe execution, Analysis Database mutation, OHLCV repair, or Data Manager mutation.
- AS12 added the backend-only read-only `AnalysisSuiteCandidateTemporalValidator` with `AnalysisSuiteTemporalValidationSegment`, `AnalysisSuiteTemporalValidationConfig`, `AnalysisSuiteCandidateSegmentValidation`, `AnalysisSuiteCandidateMetricDegradation`, `AnalysisSuiteCandidateTemporalValidationResult`, and `AnalysisSuiteCandidateTemporalValidationReport`. It builds `chronological_holdout` discovery/validation cohort splits from `anchor_ts_ms`, preserves positive/negative cohort roles, retests AS11 candidates through AS10 `AnalysisSuiteWhiteBoxRuleTester.test_rule(...)`, does not reimplement AS10 metric formulas, reports segment-level metrics, metric degradation, diagnostic stability score, survival statuses (`survived`, `degraded`, `failed`, `insufficient_data`, `unknown`), AS11 multiple-testing warning propagation, sample-limited warning propagation, and bounded candidate counts with default `100` / max `500`. It does not add GUI controls, persistence, stores, candidate rescanning, new candidate mining, conjunction expansion, model logic, signals, backtesting, PnL/profit validation, trading execution, artifact calculation, recipe execution, direct dataframe/manifest policy reads, Analysis Database mutation, OHLCV repair, or Data Manager mutation.
...
- Treat Analysis Suite AS1-AS12 plus AS-GUI-1, AS-GUI-2, and AS-GUI-3 as the accepted read-only foundation for readiness, bounded dataframe preview, target/label preview, feature-set leakage prevention, diagnostic setup coherence, GUI target/feature/diagnostic setup inspection, backend and GUI POI/family occurrence preview, backend plus GUI genome/path preview, backend white-box rule testing, backend bounded rule candidate scanning, and backend candidate temporal validation.
- Treat AS8-AUDIT and AS9-AUDIT as the accepted design boundary for broader POI / event-family / road / genome architecture, treat AS10-AUDIT as the accepted architecture boundary for POI family comparison and white-box rule testing/discovery guardrails, treat AS11-AUDIT as the accepted architecture boundary for bounded candidate discovery, predicate vocabulary, ranking, pruning, multiple-testing warnings, and stability design, and treat AS12-AUDIT as the accepted architecture boundary for candidate validation, temporal stability, chronological holdout discipline, rule survival, metric degradation, and stability scoring.
- Treat AS-GUI-AUDIT as the accepted GUI exposure boundary, AS-GUI-1 as the accepted target/feature/diagnostic setup GUI, AS-GUI-2 as the accepted POI/family preview GUI, and AS-GUI-3 as the accepted genome path preview GUI. Next Analysis Suite step after AS12D docs sync: AS13-AUDIT for validated rule review / promotion architecture. GUI polish may be handled separately if manual exploration finds usability issues.
...
Change log
------------------------------------------------------------
v0.52: Analysis Suite AS12 backend sync. Documents the accepted backend-only read-only `AnalysisSuiteCandidateTemporalValidator`, including chronological_holdout split construction from `anchor_ts_ms`, positive/negative cohort preservation, AS10 rule tester reuse for per-segment metrics, metric degradation diagnostics, diagnostic stability score, survival statuses, AS11 multiple-testing warning propagation, sample-limited warning propagation, bounded candidate validation counts, no GUI exposure, no persistence/stores, no candidate rescanning, no new candidate mining, no conjunction expansion, no model/training/signal/backtesting/PnL/trading-execution/artifact/recipe/database/OHLCV/Data Manager mutation behavior, and AS13-AUDIT as the next architecture step.

v0.51: Analysis Suite AS12-AUDIT design. Documents Candidate Validation / Temporal Stability architecture in `docs/DESIGN_analysis_suite.md`, including candidate validation, temporal stability, `chronological_holdout` as the recommended first split, validation split types, rule survival statuses, metric degradation, stability scoring, segment-level diagnostics, AS10/AS11 relationships, sample-limited validation warnings, multiple-testing risk preservation, candidate promotion/demotion statuses, future backend MVP boundaries, and future AS12/AS12D/AS13-AUDIT sequence. Preserves no-implementation, no-GUI, no-persistence, no-stores, no-model/training/signal/backtesting/PnL/trading-execution/artifact/recipe/database/OHLCV/Data Manager mutation boundaries.

v0.50: Analysis Suite AS11 backend sync. Documents the accepted backend-only read-only `AnalysisSuiteRuleCandidateScanner`, including AS9-derived predicate vocabulary generation inside AS10 comparison cohorts, bounded single-predicate scans, AS10 rule tester reuse for metrics, support/precision/lift filtering and diagnostic ranking, scanned/returned/filtered candidate counts, sample-limited warnings, no multiple-testing correction warning, `stability_status = "unknown"`, no GUI exposure, no persistence/stores, no conjunction expansion, no broad mining, no model/training/signal/backtesting/PnL/trading-execution/artifact/recipe/database/OHLCV/Data Manager mutation behavior, and AS12-AUDIT as the next architecture step.

v0.49: Analysis Suite AS11-AUDIT design. Documents White-Box Rule Candidate Discovery / Bounded Mining architecture in `docs/DESIGN_analysis_suite.md`, including bounded predicate vocabulary, single-predicate scan architecture, cautious conjunction expansion, candidate ranking and pruning, multiple-testing and false-discovery warnings, temporal split and stability concepts, family separation, explainability requirements, AS8/AS9/AS10 relationships, and the recommended backend-only AS11 single-predicate scan MVP. Preserves no-implementation, no-GUI, no-persistence, no-stores, no-model/training/signal/backtesting/PnL/trading-execution/artifact/recipe/database/OHLCV/Data Manager mutation boundaries. Marks AS11 backend implementation and AS12 temporal-stability audit as future work.

v0.48: Analysis Suite AS10 backend sync. Documents the accepted backend-only read-only `AnalysisSuiteWhiteBoxRuleTester`, including explicit predicate/rule testing over AS9 genome paths, explicit positive/negative comparison cohorts, supported predicate operators, path-offset leakage guardrails, AS7 diagnostic gating, support/coverage/precision/recall/specificity/false-positive/false-negative/lift metrics, bounded-preview sample-limit reporting, no GUI exposure, no persistence/stores, no broad rule discovery, no candidate mining, no model/training/signal/backtesting/PnL/trading-execution/artifact/recipe/database/OHLCV mutation behavior, and AS11-AUDIT as the next architecture step.

v0.47: Analysis Suite AS10-AUDIT design. Documents POI Family Comparison and White-Box Rule Discovery architecture in `docs/DESIGN_analysis_suite.md`, including white-box rule definitions, rule candidates, comparison sets, road/outcome/false-signal concepts, support/coverage/precision/recall/lift/false-positive/false-negative/stability metrics, rule-testing versus rule-discovery boundaries, AS5/AS7/AS8/AS9 relationships, and the recommended backend-only AS10 rule-testing MVP. Preserves no-implementation, no-GUI, no-persistence, no-stores, no-model/training/signal/backtesting/PnL/trading-execution/artifact/recipe/database/OHLCV mutation boundaries. Marks AS10 backend implementation and AS11 discovery/candidate-mining audit as future work.

v0.46: Analysis Suite AS-GUI-3 docs sync. Documents the accepted Genome Path Preview UI in `AnalysisSuiteWindow`, including the `Genome Path Preview` tab, AS9 encoding definition controls, genome component rows, `AnalysisSuiteGenomePathBuilder.validate_encoding_definition(...)`, `preview_paths(...)`, and `preview_paths_for_poi_family(...)` service ownership, row-anchored and AS8-family-anchored path preview rendering, stale genome state clearing, no persistence, no direct dataframe or manifest policy reads, no Dynamic Binner fitting, no full Variation Analyzer, no AS10 controls, and no white-box/rule-discovery/model/signal/backtesting/database mutation behavior. Marks AS10-AUDIT as the next Analysis Suite architecture step.

v0.45: Analysis Suite AS-GUI-2 docs sync. Documents the accepted POI / Family Preview UI in `AnalysisSuiteWindow`, including the `POI / Family Preview` tab, AS8 event-kind controls, family condition rows, `AnalysisSuitePoiFamilyPlanner.preview_poi_occurrences(...)` and `preview_family(...)` service ownership, stale POI/family state clearing, no persistence, no direct dataframe or manifest policy reads, no AS9 genome path controls, no category-builder UI, and no model/signal/backtesting/database mutation behavior. Marks AS-GUI-3 or AS10-AUDIT as future options.

v0.44: Analysis Suite AS-GUI-1 docs sync. Documents the accepted Target / Feature / Diagnostic Preview UI in `AnalysisSuiteWindow`, including Data Preview, Target Preview, Feature Set, and Diagnostic Report tabs; AS5/AS6/AS7 service ownership; stale-state clearing; no persistence; no direct dataframe or manifest policy reads; no AS8/AS9 controls; manual smoke-check acceptance; and layout/small polish as future work.

v0.43: Analysis Suite GUI audit sync. Documents AS-GUI-AUDIT as the accepted GUI exposure boundary, confirmed the then-current `AnalysisSuiteWindow` as a read-only AS1 catalog plus AS3 bounded preview surface, recommended AS-GUI-1 as Target / Feature / Diagnostic Preview UI for AS5/AS6/AS7, staged AS-GUI-2 for AS8 POI/family previews and AS-GUI-3 for AS9 genome path previews, and preserved no-GUI-implementation/no-persistence/no-stores/no-model/no-signal/no-database-mutation boundaries.

v0.42: Analysis Suite AS9 Genome Path Builder sync. Documents `AnalysisSuiteGenomePathBuilder`, genome component/encoding/snapshot/path preview models, public preview methods, supported encodings, row-anchored and AS8-family-anchored path previews, AS1/AS7 gating, AS6/manifest metadata validation, bounded JSON-safe samples, and the boundary that AS9 does not add GUI controls, persistence, genome stores, Dynamic Binner fitting, full Dynamic Binner, full Variation Analyzer, white-box discovery, rule mining, backtesting, model logic, signals, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation. Marks AS-GUI-AUDIT as the next Analysis Suite audit step before deeper AS10 work.

v0.41: Analysis Suite AS9-AUDIT design. Expands `docs/DESIGN_analysis_suite.md` with Genome Encoding / Variation Analyzer / Dynamic Binner integration architecture: genome definitions, genome components, encoding policy, genome snapshots and paths, Variation Analyzer and Dynamic Binner roles, leakage and knowability rules, relationships to AS5-AS8, and the recommended backend-only AS9 Genome Path Builder MVP. Backend services, GUI controls, persistence, stores, white-box rule discovery, backtesting, models, signals, artifact calculation, recipe execution, OHLCV repair, and Analysis Database mutation remain out of scope.

v0.40: Analysis Suite AS8 POI/family planner sync. Documents `AnalysisSuitePoiFamilyPlanner`, POI and family preview models, public preview methods, supported event kinds and condition operators, AS1/AS7 gating, manifest-metadata validation, AS5-style leakage rejection, bounded JSON-safe samples, and the boundary that AS8 does not add GUI controls, persistence, POI stores, road classification, genome paths, Dynamic Binner or Variation Analyzer implementation, white-box discovery, backtesting, model logic, signals, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.

v0.39: Analysis Suite AS8-AUDIT design. Adds `docs/DESIGN_analysis_suite.md` defining the World Line, POIs, event families/subfamilies, roads, outcomes, false-signal classes, genomes, genome paths, pre/post-event windows, AS1-AS7 relationships, and the recommended backend-only AS8 POI Definition and Family Planner MVP. Backend services, GUI controls, persistence, POI discovery, white-box discovery, backtesting, models, signals, artifact calculation, recipe execution, OHLCV repair, and Analysis Database mutation remain out of scope.

v0.38: Analysis Suite diagnostic report AS7 sync. Documents `AnalysisSuiteDiagnosticReportService`, `AnalysisSuiteDiagnosticReport`, and `AnalysisSuiteFeatureColumnDiagnostic`; AS1/AS5/AS6 report composition; `ready` / `warning` / `blocked` / `error` diagnostic statuses; dataset, target, feature-set, leakage, label availability, missingness, and dtype diagnostics; AS6-accepted selected-feature dataframe consistency checks; and the boundary that raw CSV headers are not feature truth. GUI wiring, persisted diagnostic reports, projects/runs/reports, models, signals, POI analysis, white-box discovery, artifact calculation, recipe execution, OHLCV repair, and Analysis Database mutation remain out of scope.

v0.37: Analysis Suite feature-set planner AS6 sync. Documents `AnalysisSuiteFeatureSetPlanner`, `AnalysisSuiteFeatureCandidate`, `AnalysisSuiteFeatureSetDefinition`, and `AnalysisSuiteFeatureSetPreviewReport`; feature set versus feature candidate semantics; manifest-derived candidate listing; selected feature validation with order preservation; eligible feature groups; AS5 leakage metadata enforcement; `ts_ms` reservation; unknown metadata blocking; current-row `close[t]` eligibility for future-return targets; AS1 `can_preview` gating; and JSON-safe group/leakage diagnostics. Feature-set persistence, `FeatureSetStore`, GUI wiring, projects/runs/reports, models, signals, trading logic, artifact calculation, recipe execution, OHLCV repair, and Analysis Database mutation remain out of scope.
