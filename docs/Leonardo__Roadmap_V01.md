...
- Treat Analysis Suite AS1-AS9 plus AS-GUI-1, AS-GUI-2, and AS-GUI-3 as the accepted read-only foundation for readiness, bounded dataframe preview, target/label preview, feature-set leakage prevention, diagnostic setup coherence, GUI target/feature/diagnostic setup inspection, backend and GUI POI/family occurrence preview, and backend plus GUI genome/path preview.
