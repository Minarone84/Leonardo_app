...
- Treat Analysis Suite AS1-AS9 plus AS-GUI-1 as the accepted read-only foundation for readiness, bounded dataframe preview, target/label preview, feature-set leakage prevention, diagnostic setup coherence, first GUI target/feature/diagnostic setup inspection, backend POI/family occurrence preview, and backend genome/path preview.
- Treat AS8-AUDIT and AS9-AUDIT as the accepted design boundary for broader POI / event-family / road / genome architecture.
- Treat AS-GUI-AUDIT as the accepted GUI exposure boundary and AS-GUI-1 as the accepted first Analysis Suite setup GUI. Next Analysis Suite step after AS-GUI-1D: either AS-GUI-POLISH-AUDIT if manual testing finds usability pain points, AS-GUI-2 for AS8 POI/family previews, or AS10-AUDIT for POI Family Comparison and White-Box Rule Discovery. AS-GUI-3 should later expose AS9 genome path previews after AS8 GUI concepts are stable.
