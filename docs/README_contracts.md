...
AS-GUI-1 adds the first interactive Analysis Suite setup contract to `AnalysisSuiteWindow`. The window exposes `Data Preview`, `Target Preview`, `Feature Set`, and `Diagnostic Report` tabs. `Data Preview` preserves AS3 bounded Head/Tail preview. `Target Preview` calls `AnalysisSuiteTargetPlanner`; `Feature Set` calls `AnalysisSuiteFeatureSetPlanner`; and `Diagnostic Report` calls `AnalysisSuiteDiagnosticReportService`. The GUI collects target and feature-selection intent and renders backend report objects only. Backend services own target generation, feature eligibility, leakage policy, dataframe reads, diagnostic coherence, missingness/dtype checks, blockers, warnings, and errors. Target definitions, feature sets, and diagnostic reports are not persisted.

AS-GUI-2 adds the read-only POI/family preview contract to `AnalysisSuiteWindow`. The window exposes `POI / Family Preview` for basic POI intent and family condition intent. POI occurrence preview calls `AnalysisSuitePoiFamilyPlanner.preview_poi_occurrences(...)`; family membership preview calls `AnalysisSuitePoiFamilyPlanner.preview_family(...)`. The GUI renders AS8 occurrence counts, sample occurrences, matched/unmatched family counts, sample memberships, condition summaries, blockers, warnings, and errors. Backend services own POI occurrence detection, family membership matching, source/condition metadata validation, leakage/source eligibility policy, and dataframe reads. POI definitions and POI family definitions are not persisted.

AS-GUI-3 adds the read-only genome/path preview contract to `AnalysisSuiteWindow`. The window exposes `Genome Path Preview` for in-memory AS9 genome encoding intent, row-anchored path preview, and POI-family-anchored path preview. Encoding validation calls `AnalysisSuiteGenomePathBuilder.validate_encoding_definition(...)`; row-anchored preview calls `AnalysisSuiteGenomePathBuilder.preview_paths(...)`; and POI-family-anchored preview calls `AnalysisSuiteGenomePathBuilder.preview_paths_for_poi_family(...)` when a current AS8 family preview report exists. The GUI renders AS9 status, row/path counts, sample paths, sample snapshots/components, blockers, warnings, and errors. Backend services own genome source validation, encoding values, variation descriptors, static bin rule application, path construction, POI/family anchoring semantics, leakage/source eligibility policy, and dataframe reads. Genome definitions, encoding definitions, and genome paths are not persisted. AS10 controls, AS11 controls, AS12 controls, category-builder UI, white-box discovery, rule mining, Dynamic Binner fitting, full Dynamic Binner, full Variation Analyzer, training, signals, backtesting, stores, Data Manager mutation, manifest writes, dataframe writes, direct GUI `dataframe.csv` reads, and direct GUI `manifest.json` policy parsing remain out of scope.

Analysis Suite target preview is also read-only. `AnalysisSuiteTargetPlanner` consumes AS1 readiness and creates in-memory `AnalysisSuiteTargetPreviewReport` diagnostics from `AnalysisSuiteTargetDefinition` rules. A target is a future-dependent value or event the Analysis Suite wants to predict, classify, measure, or explain; a label is the generated per-row target series aligned back to timestamp `t`, so features at `t` predict the label at `t`. AS5 supports future return regression, `(close[t + N] - close[t]) / close[t]`, and future direction classification, `up` / `down` / `flat` from explicit thresholds. `horizon_bars` must be positive, means `N` dataframe rows forward in the Analysis Database timeframe, keeps labels aligned to `t`, records `label_end_ts_ms` for the future row used, and marks the last `N` rows unavailable. The planner gates dataframe access through AS1 `can_preview`; non-strict previewable datasets may run while preserving readiness warnings and blockers. Reports include database identity, target definition, row count, available/unavailable label counts, first/last available timestamps, regression stats or class distribution, sample rows, blockers/warnings/errors, and leakage metadata. Label outputs are `target_only`, `future_derived`, and `feature_eligible = false`; they are not persisted, not written to `dataframe.csv`, not ordinary feature columns, and AS6 feature-set planning must reject target-only/future-derived outputs. AS5 does not add TargetDefinitionStore, label files, Analysis Project/Run/Report stores, model training, signals, trading logic, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.
...
AS9 gates through AS1 readiness or optional AS7 diagnostic reports, blocks `can_preview == false`, blocks blocked/error AS7 diagnostics, and allows non-strict previewable datasets while preserving warnings. It validates component source columns through AS6 feature-set reports or manifest metadata when available, blocks `ts_ms`, target-only, future-derived, and `feature_eligible = false` inputs when metadata exists, reads dataframe values only for read-only genome/path preview, and does not treat raw CSV headers as semantic genome truth. Row-anchored paths preserve ordered snapshots as `G(t-k) ... G(t)`, invalid or early anchors are reported with structured blockers, and optional AS8 POI/family anchoring uses matched family membership samples without recomputing POIs. AS9 and AS-GUI-3 do not persist genome definitions, encoding definitions, or paths; do not add genome stores; do not implement Dynamic Binner fitting, full Dynamic Binner, full Variation Analyzer, road classification, outcome scoring, white-box rule discovery, rule mining, model training, signals, backtesting, artifact calculation, recipe execution, OHLCV repair, or Analysis Database mutation.

Analysis Suite white-box rule testing is a read-only AS10 consumer contract. `AnalysisSuiteWhiteBoxRuleTester` evaluates explicit in-memory rules against supplied AS9 genome-path cohorts and returns bounded JSON-safe diagnostics. The contract objects are `AnalysisSuiteRulePredicate`, `AnalysisSuiteWhiteBoxRuleDefinition`, `AnalysisSuiteComparisonCohort`, `AnalysisSuiteComparisonSetDefinition`, `AnalysisSuitePredicateResult`, `AnalysisSuiteRuleMatch`, `AnalysisSuiteRuleMetricSummary`, `AnalysisSuiteRuleValidationReport`, and `AnalysisSuiteRuleTestReport`. Public methods are `validate_rule_definition(...)`, `test_rule(...)`, and `build_comparison_set_from_path_reports(...)`.

AS10 predicates support `equals`, `not_equals`, `gt`, `gte`, `lt`, `lte`, `in`, `not_in`, `is_null`, and `not_null`. MVP rules use AND semantics only; there are no nested expression trees, OR groups, executable predicates, arbitrary expression evaluation, or `eval`. Predicate `path_offset` selects the AS9 genome path snapshot under test: `0` is the anchor snapshot, negative offsets address previous snapshots, and positive offsets are blocked to prevent future-snapshot leakage. Missing required components fail the predicate and produce structured predicate result details.

AS10 comparison sets require explicit positive and negative/background cohorts. The tester does not silently invent negative examples and does not rebuild AS8 POI/family previews or AS9 genome paths. Empty rules and empty cohorts block. Cohort overlap and missing time-split metadata are reported as diagnostics when detected. Metrics include true positives, false positives, false negatives, true negatives, support, coverage, precision, recall, specificity, false-positive rate, false-negative rate, baseline positive rate, matched positive rate, and lift, with zero-denominator safety and small-sample warnings. Metrics are evidence-quality diagnostics rather than trading performance. If cohorts are built from bounded AS9 previews, AS10 reports `evaluated_sample_limited`, `input_path_count`, `evaluated_path_count`, and sample-limit metadata so preview metrics are not treated as full-cohort proof.

AS10 preserves AS7 diagnostic gating: blocked/error diagnostics block testing, warning diagnostics allow testing with warning, and ready diagnostics allow testing. AS10 does not persist rule definitions, comparison sets, or rule reports; does not create rule stores or Analysis Project/Run/Report stores; does not expose GUI controls; does not perform broad rule discovery, conjunction mining, or candidate mining; does not read `dataframe.csv` directly; does not parse `manifest.json` for policy; does not train models, generate signals, backtest, validate PnL/profit, generate orders, execute trades, calculate artifacts, execute recipes, build/rebuild/materialize Analysis Databases, repair OHLCV, mutate Data Manager objects, edit manifests, or write dataframes.

Analysis Suite bounded rule candidate scanning is a read-only AS11 consumer contract. `AnalysisSuiteRuleCandidateScanner` builds a finite predicate vocabulary from AS9 genome path components supplied through AS10 comparison cohorts, converts each vocabulary item into a one-predicate AS10 rule, and delegates matching and metric computation to `AnalysisSuiteWhiteBoxRuleTester.test_rule(...)`. The contract objects are `AnalysisSuitePredicateVocabularyItem`, `AnalysisSuitePredicateVocabularyReport`, `AnalysisSuiteRuleCandidate`, `AnalysisSuiteRuleCandidateScanConfig`, and `AnalysisSuiteRuleCandidateScanReport`.

AS11 defaults to `equals` predicates for symbolic, boolean, categorical, and static-bin-like observed values. Configured scans may include `not_equals`, `is_null`, and `not_null`. Numeric threshold mining is skipped by default when `numeric_threshold_policy = "none"`; AS11 does not fit quantiles, adaptive thresholds, Dynamic Binner policies, or Variation Analyzer rules. Positive path offsets are blocked, negative offsets are used only when explicitly enabled, and components marked leaky or future-derived by metadata are skipped. Candidate filtering uses blockers, `min_total_support`, `min_positive_matches`, optional `min_precision`, and optional `min_lift`. Ranking uses a diagnostic lift/precision/support-weight score and is not trading performance, alpha, profitability, or a signal.

AS11 reports `candidate_count_scanned`, `candidate_count_returned`, `candidate_count_filtered`, sample-limited input warnings, no multiple-testing correction warning, and `stability_status = "unknown"` because temporal stability validation is not performed in AS11 MVP. Returned candidate output is bounded with default `100` and max `500`. AS11 does not expose GUI controls; does not persist candidate rules, scan configs, or scan reports; does not create candidate stores or Analysis Project/Run/Report stores; does not perform conjunction expansion, broad mining, itemset mining, recursive search, genetic programming, model training, signals, backtesting, PnL/profit validation, order generation, trading execution, artifact calculation, recipe execution, Analysis Database mutation, OHLCV repair, Data Manager mutation, manifest writes, dataframe writes, direct `dataframe.csv` reads, direct `manifest.json` policy parsing, or duplicate AS10 metric formulas.

Analysis Suite candidate temporal validation is a read-only AS12 consumer contract. `AnalysisSuiteCandidateTemporalValidator` validates already-discovered AS11 candidates across chronological discovery and validation segments built from AS10 comparison cohorts. The contract objects are `AnalysisSuiteTemporalValidationSegment`, `AnalysisSuiteTemporalValidationConfig`, `AnalysisSuiteCandidateSegmentValidation`, `AnalysisSuiteCandidateMetricDegradation`, `AnalysisSuiteCandidateTemporalValidationResult`, and `AnalysisSuiteCandidateTemporalValidationReport`.

AS12 builds `chronological_holdout` splits from cohort path `anchor_ts_ms`, preserves positive and negative cohort roles, and calls AS10 `AnalysisSuiteWhiteBoxRuleTester.test_rule(...)` for each candidate rule in each segment. It does not reimplement AS10 support, precision, recall, lift, false-positive, false-negative, baseline-rate, sample-limit, or diagnostic-gating formulas. AS12 reports segment-level AS10 rule-test metrics, metric degradation diagnostics for precision, lift, recall, support, and false-positive-rate behavior, diagnostic stability scores, and survival statuses: `survived`, `degraded`, `failed`, `insufficient_data`, and `unknown`. It preserves AS11 multiple-testing warnings and sample-limited warnings, handles zero denominators safely, caps validated candidates at default `100` and max `500`, and reports `candidate_count_input`, `candidate_count_validated`, `candidate_count_survived`, `candidate_count_degraded`, `candidate_count_failed`, `candidate_count_insufficient_data`, and `candidate_count_unknown`.

AS12 validates candidate robustness only. It does not expose GUI controls; does not persist validated candidates or validation reports; does not create validation stores or Analysis Project/Run/Report stores; does not rescan candidates, mine new candidates, perform conjunction expansion, train models, generate signals, backtest, validate PnL/profit, generate orders, execute trades, calculate artifacts, execute recipes, build/rebuild/materialize Analysis Databases, repair OHLCV, mutate Data Manager objects, edit manifests, write dataframes, read `dataframe.csv` directly, parse `manifest.json` directly for policy, or duplicate AS10 metric formulas.
