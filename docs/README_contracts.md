...
AS-GUI-2 adds the read-only POI/family preview contract to `AnalysisSuiteWindow`. The window exposes `POI / Family Preview` for basic POI intent and family condition intent. POI occurrence preview calls `AnalysisSuitePoiFamilyPlanner.preview_poi_occurrences(...)`; family membership preview calls `AnalysisSuitePoiFamilyPlanner.preview_family(...)`. The GUI renders AS8 occurrence counts, sample occurrences, matched/unmatched family counts, sample memberships, condition summaries, blockers, warnings, and errors. Backend services own POI occurrence detection, family membership matching, source/condition metadata validation, leakage/source eligibility policy, and dataframe reads. POI definitions and POI family definitions are not persisted. AS9 genome/path controls, genome encoding controls, category-builder UI, white-box discovery, training, signals, backtesting, stores, Data Manager mutation, manifest writes, dataframe writes, direct GUI `dataframe.csv` reads, and direct GUI `manifest.json` policy parsing remain out of scope.
