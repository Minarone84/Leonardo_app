...
The GUI may display this plan, but it must not build or reinterpret the range locally. The supported GUI path submits plain intent through CoreBridge; CoreBridge resolves the configured historical root and constructs downloader request objects internally.
...
• does not resolve the active historical root or construct downloader requests directly; CoreBridge resolves `Path(ctx.config.runtime.data_dir) / "historical"` and builds downloader requests internally
• submits preflight and confirmed download intent through CoreBridge
