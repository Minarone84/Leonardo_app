...
AS12-AUDIT defines candidate validation and temporal stability architecture. It treats validation as diagnostic retesting of AS11 candidates across independent or chronologically separated data segments. The recommended first validation split is `chronological_holdout`, with an earlier discovery/reference segment and a later validation/holdout segment. AS12 design reserves split types for `discovery_validation`, `rolling_windows`, `expanding_windows`, `recent_holdout`, and `custom_segments`, but no AS12 backend is implemented yet.

AS12 design defines rule survival statuses (`survived`, `degraded`, `failed`, `insufficient_data`, and `unknown`), metric degradation for support, precision, recall, lift, false-positive behavior, score, and rank, and segment-level reports with timestamps, cohort counts, AS10 metric summaries, blockers, and warnings. Future AS12 should consume AS10 comparison sets and AS11 candidate reports, split cohorts by `anchor_ts_ms`, reuse `AnalysisSuiteWhiteBoxRuleTester` for per-segment metrics, propagate sample-limited and multiple-testing warnings, and avoid rescanning candidates or duplicating AS10 metric formulas. AS12-AUDIT is design-only: it does not add GUI controls, persistence/stores, saved validated rules, model training, signals, backtesting, PnL/profit validation, trading execution, artifact calculation, recipe execution, direct dataframe or manifest policy reads, Analysis Database mutation, OHLCV repair, or Data Manager mutation.
