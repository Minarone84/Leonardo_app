...
AS11-AUDIT is the design-only bounded white-box rule candidate discovery architecture. It defines a finite AS9-derived predicate vocabulary, support-first single-predicate scan, cautious future conjunction expansion, candidate ranking and pruning, multiple-testing and false-discovery warnings, temporal split and stability concepts, family separation diagnostics, explainability requirements, and AS8/AS9/AS10 ownership boundaries. The recommended future backend MVP is a backend-only, read-only, non-persistent single-predicate scanner that consumes AS10 comparison sets and AS9 genome paths, reuses `AnalysisSuiteWhiteBoxRuleTester` for metrics, returns bounded JSON-safe candidate reports with default `100` and max `500` candidates, and reports candidate counts, support thresholds, sample-limit warnings, and stability status. AS11-AUDIT does not implement the backend, GUI controls, persistence, stores, broad conjunction mining, model training, signals, backtesting, PnL/profit validation, trading execution, artifact calculation, recipe execution, Analysis Database mutation, OHLCV repair, or Data Manager mutation.
