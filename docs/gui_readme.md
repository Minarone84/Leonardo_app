...
Allowed actions are `Refresh Catalog`, row detail display, the `Data Preview`, `Target Preview`, `Feature Set`, `Diagnostic Report`, and `POI / Family Preview` tabs, `Open Data Manager`, and close. `Open Data Manager` is routing only; Analysis Suite does not perform repair, build, update, or calculation work itself.
...
The Analysis Suite GUI does not own readiness, preview, target, feature eligibility, leakage, diagnostic, POI/family, or dataframe-read policy. Policy remains in `AnalysisSuiteDatasetReadinessService`, `AnalysisSuiteDataframePreviewService`, `AnalysisSuiteTargetPlanner`, `AnalysisSuiteFeatureSetPlanner`, `AnalysisSuiteDiagnosticReportService`, and `AnalysisSuitePoiFamilyPlanner`; the GUI must not inspect `manifest.json`, `dataframe.csv`, or `source_ohlcv.snapshot` to classify readiness, feature/leakage, or POI/family policy, and it must not load `dataframe.csv` directly or call `AnalysisDatabaseStore.load_dataframe(...)`. Previewable does not mean analysis-ready: non-strict datasets may be previewed only when AS1 allows it, with warnings and blockers still visible.

Selection changes clear stale state: changing the selected Analysis Database clears target, feature, diagnostic, POI, and family previews; changing target settings clears downstream feature and diagnostic state; changing feature selections clears diagnostic and POI/family state; changing POI definition inputs clears stale occurrence/family reports; and changing family condition inputs clears stale family reports. Initial manual GUI exploration after AS-GUI-1 found that workflow acceptable for continued work. Layout and small usability polish remain future work and are not considered complete.

Still out of scope for the GUI: AS9 genome/path controls, genome encoding controls, category-builder controls, white-box rule discovery, model training, signal generation, backtesting, target/label/feature-set/diagnostic-report/POI/family/genome persistence, POI/family stores, genome stores, `FeatureSetStore`, Analysis Projects, Analysis Runs, Analysis Reports, trading workflows, road classification, outcome distribution analysis, artifact calculation, recipe execution, Analysis Database build/rebuild/materialization, component editing, database extension, raw OHLCV repair/validation, manifest writes, dataframe writes, and GUI-owned leakage, feature-eligibility, source-eligibility, or POI/family validation policy.

Recommended staging after AS-GUI-2 is AS-GUI-3 for Genome Path Preview UI or AS10-AUDIT for POI family comparison and white-box rule-discovery architecture. Future Analysis Suite GUI work should keep backend services as the policy owners; GUI code must not infer feature eligibility from raw CSV headers, compute labels, compute POI occurrences or family memberships, duplicate leakage or source/condition policy, load `dataframe.csv` directly, write manifests/dataframes, persist definitions/reports, or perform Data Manager mutation.
