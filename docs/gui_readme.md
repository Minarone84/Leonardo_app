...
Allowed actions are `Refresh Catalog`, row detail display, the `Data Preview`, `Target Preview`, `Feature Set`, and `Diagnostic Report` tabs, `Open Data Manager`, and close. `Open Data Manager` is routing only; Analysis Suite does not perform repair, build, update, or calculation work itself.
...
The Analysis Suite GUI does not own readiness, preview, target, feature eligibility, leakage, diagnostic, or dataframe-read policy. Policy remains in `AnalysisSuiteDatasetReadinessService`, `AnalysisSuiteDataframePreviewService`, `AnalysisSuiteTargetPlanner`, `AnalysisSuiteFeatureSetPlanner`, and `AnalysisSuiteDiagnosticReportService`; the GUI must not inspect `manifest.json`, `dataframe.csv`, or `source_ohlcv.snapshot` to classify readiness or feature/leakage policy, and it must not load `dataframe.csv` directly or call `AnalysisDatabaseStore.load_dataframe(...)`. Previewable does not mean analysis-ready: non-strict datasets may be previewed only when AS1 allows it, with warnings and blockers still visible.

Selection changes clear stale state: changing the selected Analysis Database clears target, feature, and diagnostic previews; changing target settings clears downstream feature and diagnostic state; changing feature selections clears the diagnostic state. Initial manual GUI exploration after AS-GUI-1 found the workflow acceptable for continued work. Layout and small usability polish remain future work and are not considered complete.

Still out of scope for the GUI: AS8 POI/family controls, AS9 genome/path controls, category-builder controls, white-box rule discovery, model training, signal generation, backtesting, target/label/feature-set/diagnostic-report/POI/family/genome persistence, POI stores, genome stores, `FeatureSetStore`, Analysis Projects, Analysis Runs, Analysis Reports, trading workflows, road classification, artifact calculation, recipe execution, Analysis Database build/rebuild/materialization, component editing, database extension, raw OHLCV repair/validation, manifest writes, dataframe writes, and GUI-owned leakage or feature-eligibility policy.
