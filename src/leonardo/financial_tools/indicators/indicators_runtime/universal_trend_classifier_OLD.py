...
def _pending_breakout_state(pending: _PendingHorizontalRangeBreakout) -> _HorizontalRangeState:
    ...
    *,
    ...
    for pos, (index, peak_value, trough_value) in enumerate(zip(df.index, peak_values, trough_values)):
        ...
        if has_peak_value and has_trough_value:
            # A row that is both peak and trough is ambiguous for directional
            # and range discovery structure. Peaks & Troughs should rarely emit
            # this, but UTC must remain deterministic if an imported artifact
            # contains it.
            continue
...
class UniversalTrendClassifierRealtime:
    ...
    def _handle_invalid_bar(self, pos: int, changed_start: int, changed_end: int) -> Tuple[int, int]:
        prev_pos = pos - 1
        if self._active_range is not None and prev_pos >= self._active_range.start_pos:
    ...
    def _update_active_range_with_bar(
        ...
        self._active_range = None
        return min(changed_start, state.start_pos), max(changed_end, bar.position)
    ...
    def _update_pending_range_breakout_with_bar(
        ...
        if bar.position <= pending.expires_pos and _bar_reclaims_pending_breakout(bar, pending):
        ...
        if bar.position > pending.expires_pos:
            return self._finalize_pending_range_breakout(changed_start, changed_end)
